...
from utils.graph_editor import dijkstra
...
class GraphApp:
    def __init__(self, root):
        ...
        self.current_menu = None  # Ссылка на текущее меню

        self.canvas.bind("<Button-1>", self.on_left_click)
        self.canvas.bind("<Button-3>", self.on_right_click)
        self.canvas.bind("<Button-2>", self.on_right_click)  # Для совместимости

        # Кнопка для запуска поиска кратчайшего пути
    ...
    def show_vertex_menu(self, vertex, x, y):
        ...
        menu.add_command(label="Установить как начальную вершину", command=lambda: self.set_start_vertex(vertex))
        menu.add_command(label="Установить как конечную вершину", command=lambda: self.set_end_vertex(vertex))
    ...
    def set_start_vertex(self, vertex):
        if self.start_vertex:
            self.canvas.itemconfig(self.start_vertex["id"], fill="lightblue")
        self.start_vertex = vertex
        self.canvas.itemconfig(vertex["id"], fill="green")

    def set_end_vertex(self, vertex):
        if self.end_vertex:
            self.canvas.itemconfig(self.end_vertex["id"], fill="lightblue")
        self.end_vertex = vertex
        self.canvas.itemconfig(vertex["id"], fill="red")
    ...
    def save_graph(self):
        ...
        messagebox.showinfo("Сохранение", "Граф сохранен в файл graph.json")
    ...
    def load_graph(self):
        try:
            ...
            messagebox.showerror("Ошибка", "Файл graph.json не найден")
